...
ALTER TABLE auth_ms.users RENAME COLUMN password TO password_hash;

-- Add missing user fields
ALTER TABLE auth_ms.users
...
UPDATE auth_ms.users SET active = enabled;
...
ALTER TABLE auth_ms.refresh_tokens
...
-- Indexes
CREATE INDEX IF NOT EXISTS idx_refresh_tokens_hash      ON auth_ms.refresh_tokens(token_hash);
CREATE INDEX IF NOT EXISTS idx_refresh_tokens_session   ON auth_ms.refresh_tokens(session_id);
CREATE INDEX IF NOT EXISTS idx_refresh_tokens_expires   ON auth_ms.refresh_tokens(expires_at);
